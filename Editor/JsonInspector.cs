...
public class JsonInspector : Editor
{
    ...
    public static void OnSelectAsset(UnityEngine.Object uobject)
    {
        ...
        if (m_AssetPath.EndsWith(".json"))
        {
            ...
            DefaultAssetInspector . onInspectorGUIHander . AddListener(jsonInspector . OnInspectorGUI);
            DefaultAssetInspector . onDestroyGUIHander   . AddListener(jsonInspector . OnDestroy);
        }
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        {
            EditorGUICustom.DrawBoxTitleColor(target.name, Color.cyan, (rect) =>
                     {
                         GUI.Label(rect, OnJs);
                     });


              back = new Color(0.5568628f, 0.7536737f, 0.7843137f, 0.3490196f);
           // back = EditorGUILayout.ColorField(back);

            EditorGUICustom.BoxColor(back, (rect) =>
            {
                if (m_select.isHas)
        ...
        {
            m_isSetDirty = false;
            File.WriteAllText(m_jsonPath, m_jSONObject.ToString());
        }
    }
